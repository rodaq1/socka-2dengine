...
namespace Engine
{
  ...
  {
  public:
    static void Register(sol::state &lua, Project *project)
    {
      RegisterCore(lua, project);
      RegisterMath(lua);
      RegisterECS(lua);
  ...
  private:
    static void RegisterCore(sol::state &lua, Project *project)
    {
      ...
      auto sceneTable = lua["Scene"].get_or_create<sol::table>();

      sceneTable["load"] = [project](const std::string &sceneName)
      {
        if (project)
        {
          project->requestSceneLoad(sceneName);
          Log::info("[Lua] Scene load requested: " + sceneName);
        }
        else
        {
          Log::error("[Lua] Project context missing. Cannot load scene.");
        }
      };

      sceneTable["getName"] = [project]() -> std::string
      {
        if (project && project->getActiveScene())
        {
          return project->getActiveScene()->getName();
        }
    ...
    static void RegisterECS(sol::state &lua)
    {
      ...
      lua.new_usertype<InputControllerComponent>(
          ...
          "mappings", &InputControllerComponent::mappings,
          "addMapping",
      ...
      lua.new_usertype<SpriteComponent>(
          ...
          // ===== Color as vec2 (rgba packed manually) =====
          "setColor",
          [](SpriteComponent &s, int r, int g, int b, int a)
          {
            s.color.r = (Uint8)r;
            s.color.g = (Uint8)g;
            s.color.b = (Uint8)b;
          ...
          // ===== Source Rect =====
          "setSourceRect",
          [](SpriteComponent &s, int x, int y, int w, int h)
          {
            s.sourceRect.x = x;
            s.sourceRect.y = y;
            s.sourceRect.w = w;
          ...
          {
            return glm::vec4(
                (float)s.sourceRect.x,
                (float)s.sourceRect.y,
                (float)s.sourceRect.w,
                (float)s.sourceRect.h);
          });
      ...
      lua.new_usertype<Entity>(
          ...
          "getVelocity", [](Entity &e)
          { return e.getComponent<VelocityComponent>(); },
          "getSprite", [](Entity &e)
          { return e.getComponent<SpriteComponent>(); },
          "getRigidbody", [](Entity &e)
          { return e.getComponent<RigidBodyComponent>(); },
          "getBoxCollider", [](Entity &e)
          { return e.getComponent<BoxColliderComponent>(); },
          "getCircleCollider", [](Entity &e)
          { return e.getComponent<CircleColliderComponent>(); },
          "getPolygonCollider", [](Entity &e)
          { return e.getComponent<PolygonColliderComponent>(); },
          "getSound", [](Entity &e)
          { return e.getComponent<SoundComponent>(); },
          "getInheritance", [](Entity &e)
          { return e.getComponent<InheritanceComponent>(); },
          "getAnimation", [](Entity &e)
          { return e.getComponent<AnimationComponent>(); },
          "getCamera", [](Entity &e)
          { return e.getComponent<CameraComponent>(); },
          "getInputController", [](Entity &e)
          { return e.getComponent<InputControllerComponent>(); });
    }
  };
} // namespace Engine