...
namespace Engine
{
    ...
    void Entity::copyAllComponentsFrom(Entity *other)
    {
        for (auto &[type, compPtr] : other->components)
        {
            std::unique_ptr<Component> cloned = compPtr->clone();
            cloned->owner = this;
            components[type] = std::move(cloned);
        }
