...
namespace Engine {
void ProjectSerializer::saveProjectFile(Project *project,
                                        const std::string &filepath) {
  if (!project) {
    Log::error("[ProjectSerializer] Error: Project pointer is null.");
    return;
  }
  ...
  projectJson["Project"] = {{"Name", config.name},
  ...
  if (file.is_open()) {
    ...
    Log::info("[ProjectSerializer] Project configuration saved: " + filepath);
  } else {
    Log::error("[ProjectSerializer] Failed to open " + filepath +
               " for writing.");
  }
}
...
void ProjectSerializer::saveScene(Scene *scene, const std::string &filepath) {
  json sceneJson;
  ...
  auto entities = scene->getEntityRawPointers();
...
json ProjectSerializer::serializeEntity(Entity *entity) {
  ...
  if (entity->hasComponent<CircleColliderComponent>()) {
    ...
    j["Components"]["CircleCollider"] = {
        {"Radius", c->radius},       {"Offset", {c->offset.x, c->offset.y}},
        {"Layer", c->layer},         {"Mask", c->mask},
        {"IsTrigger", c->isTrigger}, {"IsStatic", c->isStatic}};
  }
...
std::string findAssetPath(const fs::path &currentDir,
  ...
  try {
    if (fs::exists(currentDir / fileName)) {
    ...
    for (const auto &entry : fs::directory_iterator(currentDir)) {
      if (entry.is_directory()) {
        std::string found =
            findAssetPath(entry.path(), fileName, depth + 1, maxDepth);
        if (!found.empty())
          return found;
      }
    }
  } catch (const fs::filesystem_error &e) {
    // Handle permission errors or broken links silently or log them
  }
