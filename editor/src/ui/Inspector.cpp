    ...
    if (m_SceneSelected && currentScene) {
        ...
        if (ImGui::CollapsingHeader("Background", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            if (bg.type == Engine::BackgroundType::Solid) {
                ...
                if (ImGui::Button("Select Image...")) { ImGui::OpenPopup("BgImagePopup"); }
        ...
        if (ImGui::CollapsingHeader("Active Camera", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            if (camera) {
                ...
                if (ImGui::DragFloat("Ortho Size", &size, 0.1f, 1.0f, 2000.0f)) {
                    camera->setOrthographicSize(size);
                }
                
                glm::vec2 camPos = camera->getPosition();
                if (ImGui::DragFloat2("Camera Position", &camPos.x, 1.0f)) {
  ...
  if (m_SelectedEntity->hasComponent<Engine::SoundComponent>()) {
    ...
    if (ImGui::CollapsingHeader("Audio Source", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        if (const ImGuiPayload* payload = ImGui::GetDragDropPayload()) {
            if (payload->IsDataType("ASSET_ID")) isDraggingAsset = true;
        }
        ...
        ImGui::PopItemWidth();
        if (isDraggingAsset) ImGui::PopStyleColor();
