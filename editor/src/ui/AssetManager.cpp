...
void EditorApp::renderAssetManager() {
    ...
    if (ImGui::Button("Assets")) currentRelativePath = "";
    ...
    for (auto& part : currentRelativePath) {
        ...
        ImGui::SameLine();
        accumulated /= part;
        if (ImGui::Button(part.string().c_str())) currentRelativePath = accumulated;
    }
    ImGui::PopStyleColor();
    ...
    if (fs::exists(currentDirectoryPath)) {
        for (auto const& entry : fs::directory_iterator(currentDirectoryPath)) {
            ...
            if (active) slotColor = IM_COL32(70, 70, 85, 255);
            ...
            if (isDir) {
                ...
                if (hovered && ImGui::IsMouseDoubleClicked(0)) currentRelativePath /= filename;
            } else {
            ...
            if (label.length() > 11) label = label.substr(0, 8) + "...";
            ImVec2 textSize = ImGui::CalcTextSize(label.c_str());
            drawList->AddText(ImVec2(pos.x + (thumbnailSize - textSize.x) * 0.5f, pos.y + thumbnailSize + 4), IM_COL32(220, 220, 220, 255), label.c_str());
...
void EditorApp::loadProjectAssets() {
    if (!m_currentProject || !m_AssetManager) return;
