  ...
  for (const auto &root : roots)
  {
    ...
    for (const auto &entry : fs::directory_iterator(root, fs::directory_options::skip_permission_denied, root_ec))
    {
      ...
      auto processFile = [&](const fs::path &filePath)
      {
        ...
        {
          Engine::Project p;
          p.setPath(filePath.parent_path().string());

          if (Engine::ProjectSerializer::loadProjectFile(&p, filePath.string()))
          {
            m_FoundProjects.push_back(p);
          }
          else
          {
            p.setName(filePath.stem().string());
            m_FoundProjects.push_back(p);
          }
        }
      };
...
Engine::Project *ProjectBrowser::render()
{
...

  {
    ...
    if (m_SelectedIndex != -1)
    {
      ...
      if (ImGui::Button("DELETE PROJECT", ImVec2(buttonWidth, buttonHeight))) {
        ImGui::OpenPopup("ConfirmDelete");
        projectToModifyIndex = m_SelectedIndex;
      }
      ImGui::PopID();
    ...
    else
    {
      ...
      ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.60f, 0.60f, 0.70f, 0.80f));
      ImGui::Text("The project folder will be created at the specified location.");
      ImGui::Spacing();
      ImGui::Text("A default project structure will be generated automatically.");
      ImGui::PopStyleColor();
      ...
      if (ImGui::Button("CREATE AND LAUNCH PROJECT", ImVec2(-1, createButtonHeight)))
      {
...
Engine::Project *ProjectBrowser::createNewProject()
{
  ...
  {
    // Construct paths
    fs::path parentPath(m_NewProjectPath);
    fs::path rootPath = fs::absolute(parentPath / m_NewProjectName);

    if (fs::exists(rootPath))
    {
      Engine::Log::error("Folder already exists: " + rootPath.string());
      return nullptr;
    }
    ...
    fs::create_directories(rootPath / config.assetDirectory);
    fs::create_directories(rootPath / "scenes");

    fs::path projectFile = rootPath / (std::string(m_NewProjectName) + Engine::Project::Extension);
    Engine::ProjectSerializer::saveProjectFile(&newProject, projectFile.string());

    Engine::FileSystem::setProjectRoot(rootPath.string());
    refreshProjectList(parentPath.string());

    for (auto &p : m_FoundProjects)
    {
      if (fs::path(p.getPath()) == rootPath)
      {
        return &p;
      }
    }
  ...
  {
    Engine::Log::error("Creation failed: " + std::string(e.what()));
    return nullptr;
  }
}